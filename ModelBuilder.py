...
def build_model(previous_prices, labels, train_factor, x):

    number_datapoints = previous_prices.shape[0]
    number_training_lines = int(number_datapoints * (1 - train_factor))
    number_previous_points = previous_prices.shape[1]

    log.info("Building model for dataset with shape %s. Train factor: %s, number of data points used for training: %s",
             previous_prices.shape, train_factor, number_training_lines
             )

    train_labels = labels[0:number_training_lines]
    train_previous_prices = previous_prices[0:number_training_lines]

    test_labels = labels[number_training_lines:]
    test_previous_prices = previous_prices[number_training_lines:]
